import { Cn as Mapping, En as Slice, Mt as isNodeEmpty, Nt as isNodeSelection, Sn as TextSelection, Tn as Fragment, X as getExtensionField, _ as callOrReturn, _n as DecorationSet, bn as PluginKey, gn as Decoration, hn as keydownHandler, i as Extension, vn as NodeSelection, wn as dropPoint, xn as Selection$1, yn as Plugin } from "./dist-jC-2Qu7K.js";
		...
		return [new Plugin({
			...
			view(view) {
				...
				const computeAndDispatch = () => {
					...
					if (prev.topPos === positions.top && prev.bottomPos === positions.bottom) return;
				...
				return {
					update(_, prevState) {
						if (view.state.doc.content.size !== prevState.doc.content.size) computeAndDispatch();
					},
					destroy: () => {
						cancelThrottle();
					...
					if (resolved.depth > 0) {
						...
						if (node.type.isTextblock && isNodeEmpty(node)) {
							const decoration = createPlaceholderDecoration({
								node,
								dataAttribute,
								hasAnchor: anchor >= nodeStart && anchor <= nodeStart + node.nodeSize,
...
//# sourceMappingURL=dist-CbjqqTm3.js.map